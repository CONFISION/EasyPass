...
4. [ ] 紧急访问
5. [ ] 全平台发布
6. [ ] 后台服务化（2026-08-22 提出）：将 EasyPass 核心作为常驻后台服务（Windows 服务或开机自启进程），浏览器扩展通过该服务读写保险库，无需启动前端 UI。动机：native messaging 每次连接都冷启动 Flutter 引擎（2-3s 延迟），且 UI 关闭时扩展也应可用。架构方案待定（常驻 host + 具名管道桥接 / 系统服务 + native messaging 转发），属底层架构改动，暂不实施。